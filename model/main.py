...
def run_simulation(h, n):
	...
	# for i in range(100):
		...
		if i // 1 >= step_next:
			...
			# SV.T_Air = float(elements_ghc[9])-3
			CV.U_ThScr = float(elements_ghc[3]) / 100
			CV.U_Roof = (float(elements_ghc[10]) + float(elements_ghc[11])) / 200
			SV.T_Can = SV.T_Air + 2
			SV.T_Top = SV.T_Air + 2
...
def main():
	...
	plt.plot(t1, label = "Air Temperature (F)")
	plt.plot(t2, label = "Outside Temperature (F)")
	plt.xlabel('Time')
	plt.legend()
	plt.show()

	plt.plot(x, label='CO2_air concentration')
	# plt.plot(y, label='CO2_top')
	plt.legend()	
	plt.xlabel('Time')
	plt.ylabel('CO2-concentration (ppm)')	
	plt.show()

	plt.plot(z, label = "Air relative humidity (%)")
	plt.xlabel('Time')
	plt.legend()
	plt.show()
